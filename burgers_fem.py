...
# Problem: dt u + u dx u = 0; u(x,0) = sin(2pi x) on [0,1] x [0,1] with periodic boundary conditions
...
#Creates time stepping
CFL = 1
tau = CFL * (h / 4 )  # CFL Computation
I = mth.floor(T / tau)
...
def Flux(u_cur):
           ...
           u_update[0] =  -  (1 / m_i) * (((u_cur[nn - 2]**2)/2) * c_10 + ((u_cur[1]**2)/2) * c_iip1) + (1/m_i)*(1/2)*(max(abs(u_cur[nn-2]), abs(u_cur[0]))*(u_cur[N-2] - u_cur[0])+ max(abs(u_cur[1]),abs(u_cur[0]))*(u_cur[1] - u_cur[0]))
           for i in range(1, nn - 1):
               u_update[i] =  - (1 / m_i) * (((u_cur[i - 1]**2)/2) * c_iim1 + ((u_cur[i + 1]**2)/2) * c_iip1) + (1/m_i)*(1/2)*(max(abs(u_cur[i-1]), abs(u_cur[i]))*(u_cur[i-1] - u_cur[i]) + max(abs(u_cur[i+1]), abs(u_cur[i]))*(u_cur[i+1] - u_cur[i]))
...
#u_exact_T = u_exact.subs(t,T) #Checks out
